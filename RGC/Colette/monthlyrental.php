...
global $query;
	...
	$query=$conn->prepare("select * from tblsalesreport WHERE YEAR(Date) = YEAR(NOW()) AND MONTH(Date) = MONTH(NOW()) AND type='Computer Rental'");
		$query->execute();
...
?>
		    ...
		    <?php
				...
				$query=$conn->prepare("select SUM(Sales) AS Total from tblsalesreport WHERE YEAR(Date) = YEAR(NOW()) AND MONTH(Date) = MONTH(NOW()) AND Type='Computer Rental' ");
		$query->execute();
