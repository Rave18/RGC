...
global $query;
	...
	$query=$conn->prepare("select * from tblsalesreport WHERE YEAR(Date) = YEAR(NOW()) AND type='Computer Rental' ORDER BY Date DESC;");
		$query->execute();
...
?>
		    ...
		    <?php
				...
				$query=$conn->prepare("select SUM(Sales) AS Total from tblsalesreport WHERE YEAR(Date) = YEAR(NOW()) AND type='Computer Rental' ORDER BY Date DESC;");
		$query->execute();
