...
global $query;
		...
		$query=$conn->prepare("select * from tblproducts where Bought>0 ");
		$query->execute();
...
?>
		    ...
		    <?php
				...
				$query=$conn->prepare("select SUM(Bought*Price) AS Total from tblproducts ");
		$query->execute();
		...
		<input type="submit" value="Reset" name="addbutton" class="btn btn-primary" ><br><br>
