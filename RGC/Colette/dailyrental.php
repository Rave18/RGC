...
global $query;
		...
		$query=$conn->prepare("select * from tblsalesreport WHERE DATE(Date) = DATE(NOW()) and Type='Computer Rental'");
		$query->execute();
...
?>
		    ...
		    <?php
				...
				$query=$conn->prepare("select SUM(Sales) AS Total from tblsalesreport WHERE DATE(Date) = DATE(NOW()) and Type='Computer Rental' ");
		$query->execute();
