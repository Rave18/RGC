...
global $query;
	...
	$query=$conn->prepare("select * from tblsalesreport WHERE YEAR(Date) = YEAR(NOW()) AND MONTH(Date) = MONTH(NOW()) AND type='product'");
		$query->execute();
...
<?php
   ...
   ?>
	...
	<h3 style="font-family:Cambria;text-align:center"><a href="dailysales.php">Daily</a>      |<a href="monthly.php">Monthly</a>|<a href="annually.php">Annually</a></h3>
...
?>
		    ...
		    <?php
				...
				$query=$conn->prepare("select SUM(Sales) AS Total from tblsalesreport WHERE YEAR(Date) = YEAR(NOW()) AND MONTH(Date) = MONTH(NOW()) AND Type='Product' ");
		$query->execute();
