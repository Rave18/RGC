...
global $query;
	...
	$query=$conn->prepare("select * from tblsalesreport WHERE DATE(Date) = DATE(NOW()) AND type='product'");
		$query->execute();
...
?>
		    ...
		    <?php
				...
				$query=$conn->prepare("select SUM(Sales) AS Total from tblsalesreport WHERE DATE(Date) = DATE(NOW()) AND Type='Product' ");
		$query->execute();
